import { PrismaClient, ProcessStatus, StepStatus, ItemType, MovementDirection, MovementReason, DeliveryVariant, OrderStatus } from '@prisma/client'
...
async function main() {
  ...
  // Ensure database is clean (for safety, though assumed by prompt)
  // NOTE: In a real environment, you'd execute a 'npx prisma migrate reset'
  // or a series of 'deleteMany' commands here if you couldn't rely on a fresh DB.

  // --- 1. Master Data (Units, Products, Workers) ---
  ...
  await prisma.templateStep.createMany({
    data: [
        { id: 1, processTemplateId: processTemplate.id, position: 1, name: "Dough Mixing", idealDurationMin: 60, requiresInput: true, instructions: "Mix until dough window is clear." },
  ...
  // Step Required Materials (createMany)
  ...
  // Process Pauses (createMany)
  await prisma.processPause.createMany({
      data: [
          { id: 1, processRunId: 3, startedAt: new Date('2025-10-16T10:30:00Z'), endedAt: null, reason: "Equipment calibration check." }
      ]
  });

  // Step Executions (createMany)
  await prisma.stepExecution.createMany({
    data: [
      // RUN 1 Steps
      { 
        id: 1, processRunId: 1, templateStepId: 1, workerId: 1, // Alice
        status: StepStatus.DONE, startedAt: new Date('2025-10-01T08:00:00Z'), finishedAt: new Date('2025-10-01T08:55:00Z'), 
        actualDurationMin: 55, inputQty: 10.00, inputUnitId: 1, notes: "A little sticky, adjusted water slightly.",
      },
      { 
        id: 2, processRunId: 1, templateStepId: 2, workerId: 1, // Alice
        status: StepStatus.DONE, startedAt: new Date('2025-10-01T09:00:00Z'), finishedAt: new Date('2025-10-01T12:00:00Z'), 
        actualDurationMin: 180, notes: "Perfect folds, stable temp.",
      },
      { 
        id: 3, processRunId: 1, templateStepId: 3, workerId: 2, // Bob
        status: StepStatus.DONE, startedAt: new Date('2025-10-01T12:00:00Z'), finishedAt: new Date('2025-10-01T12:30:00Z'), 
        actualDurationMin: 30,
      },
      // RUN 2 Step
      { 
        id: 4, processRunId: 2, templateStepId: 1, workerId: 2, // Bob (Responsible)
        status: StepStatus.BLOCKED, startedAt: new Date('2025-10-02T09:00:00Z'), finishedAt: new Date('2025-10-02T10:00:00Z'), 
        actualDurationMin: 60, notes: "Halted due to batch cancellation.",
      },
      // RUN 3 Step
      { 
  ...
  // Step Participants (createMany)
  await prisma.stepParticipant.createMany({
    data: [
      { id: 1, stepExecutionId: 4, workerId: 1, guestId: null, roleInStep: "Helper", startedAt: new Date('2025-10-02T09:15:00Z'), finishedAt: new Date('2025-10-02T10:00:00Z') }, // Alice
      { id: 2, stepExecutionId: 4, workerId: null, guestId: guestCharlie.id, roleInStep: "Observer", startedAt: new Date('2025-10-02T09:30:00Z'), finishedAt: new Date('2025-10-02T10:00:00Z') }, // Charlie
    ]
  });

  // Step Material Usages (createMany)
  ...
  // Inventory Movements (createMany)
